...
class Model_Items_Themes extends Model_Items {
	...
	public static function editItemUpdate($id, $data) {
		...
		$db->update('items', array(
			...
			'weekly_from' => ( $data['weekly_from'] ? WM_Date::format($data['weekly_from'], 'yy-mm-dd') : '0000-00-00' ),
			'weekly_to' => ( $data['weekly_to'] ? WM_Date::format($data['weekly_to'], 'yy-mm-dd') : '0000-00-00' )
		), array('id = ?' => (int)$id));
