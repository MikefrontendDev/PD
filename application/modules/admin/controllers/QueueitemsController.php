...
class QueueitemsController extends JO_Action {
	
	public static function config() {
		return array(
			'name' => self::translate('Items for approval'),
			'has_permision' => true,
    	...
    	if($items) {
    		foreach($items AS $item) {
    			$this->view->items[] = array(
    				...
    				'edit' => $request->getModule() . '/queueitems/edit/?m='.$item['module'].'&id=' . $item['id'] . $url . $url1 .$url2,
    				'comments_href' => $request->getModule() . '/queueitems/comments/?id=' . $item['id'] . $url . $url1 .$url2,
    			);
    		}
    	}
	...
	public function deleteAction() {
		...
		Model_Items::deleteItem($this->getRequest()->getPost('id'));
	}
