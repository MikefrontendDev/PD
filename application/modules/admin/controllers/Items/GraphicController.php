...
class Items_GraphicController extends JO_Action {
	...
	public function editAction() {
		...
		if($request->isPost()) {
			...
			if($request->getPost('submit')) {
	    		Model_Items_Graphic::editItem($request->getQuery('id'), $request->getParams());;
	    		$this->session->set('successfu_edite', true);
			} elseif($request->getPost('delete')) {
				Model_Items_Graphic::deleteItem($request->getQuery('id'));
