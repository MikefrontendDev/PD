...
class Items_ThemesController extends JO_Action {
	...
	public function editAction() {
		...
		if($request->isPost()) {
			...
			if($request->issetPost('submit')) {
	    		Model_Items_Themes::editItem($request->getQuery('id'), $request->getParams());;
	    		$this->session->set('successfu_edite', true);
			} elseif($request->issetPost('delete')) {
				Model_Items::deleteItem($request->getQuery('id'));
	    		$this->session->set('successfu_delete', true);
			}
    		
    		$url = '';
//			if($request->getQuery('m')) { $url .= '&m=' . $request->getQuery('m'); }
