...
class Queueitems_ImageController extends JO_Action {
	...
	public function editAction() {
		...
		if($request->isPost()) {
			if($request->getPost('submit')) {
	    		...
	    		Model_Items_Image::editItem($request->getQuery('id'), $request->getParams());
	...
	private function getForm() {
		...
		if($sizes) {
			...
			if($info['main_file'] && file_exists(BASE_PATH.'/uploads/'.$info['main_file'])) {
				$info = getimagesize(BASE_PATH.'/uploads/'.$info['main_file']);
				$filesize = $info[0]*$info[1];
			}
			
			foreach($sizes AS $k => $size) {
				if($filesize && ($size['size']*1000000) <= $filesize) {
					$size['post_price'] = $request->getPost('price['.$size['id'].']');
					$size['size'] = round($size['size'], 2);
					$this->view->sizes[$k] = $size;
					$this->view->default_price = $this->view->default_price ? $this->view->default_price : $size['id'];
				}
			}
		}
