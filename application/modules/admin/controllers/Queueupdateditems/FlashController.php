...
class Queueupdateditems_FlashController extends JO_Action {
	...
	public function editAction() {
		...
		if($request->isPost()) {
			if($request->getPost('submit')) {
	    		...
	    		Model_Items_Flash::editItemUpdate($request->getQuery('id'), $request->getParams());
	    		$this->session->set('successfu_edite', true);
			} elseif($request->getPost('delete')) {
				Model_Items::deleteItemUpdate($request->getQuery('id'));
...
//    		$this->view->tags = Model_Items::getItemTagsUpdate($id);
    	...
    	if($request->getPost('preview')) {
    		$this->view->preview = $request->getPost('preview');
    	} else {
    		$this->view->preview = $info['preview'];
    	}
