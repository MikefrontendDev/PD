...
class WM_Currency {
	...
	public static function getCurrencySymbol($right = false) {
		...
		if($right)
			self::$currencies[self::$code]['symbol_right'];
	...
	public static function setCurrency($currency) {
		...
		if(JO_Request::getInstance()->getCookie('currency') != $currency) {
			setcookie('currency', $currency, time() + 60 * 60 * 24 * 30, '/', JO_Request::getInstance()->getDomain());
		}
	}
	...
	public static function updateCurrencies($code = null) {
		...
		if (extension_loaded('curl')) {
			...
			if($data) {
				...
				if (ini_get('allow_url_fopen')) {					$content = file_get_contents('http://download.finance.yahoo.com/d/quotes.csv?s=' . implode(',', $data) . '&f=sl1&e=.csv');
				} else {
