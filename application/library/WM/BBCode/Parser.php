...
class WM_BBCode_Parser {
	...
	public function addBBCode($tagName, $replace, $useOption = false, $parseContent = true, $nestLimit = -1) {
		...
		$code = new WM_BBCode_CodeDefinition();
	...
	public function parse( $str ) {
		...
		$tokenManager = new WM_BBCode_TokenManager( $str );
		...
		while( $tokenManager->hasCurrent() ) {
			...
			if( $tokenManager->getCurrent() == "[") {
				...
				$node = new WM_BBCode_TextNode( $tokenManager->getCurrent() );
				$node->setNodeId($nodeid++);
				$parent->addChild( $node );
			}
	...
	protected function reset() {
		...
		$this->treeRoot = new WM_BBCode_DocumentElement();	
	}
	...
	public function loadDefaultCodes() {
		...
		$this->addBBCode("s", "<del>{param}</del>");
		...
		$this->addBBCode("quote", '<span class="quote">{param}</span><br/>');
		$this->addBBCode("code", '<span class="code">{param}</span><br/>', false, false, 1);
	}
